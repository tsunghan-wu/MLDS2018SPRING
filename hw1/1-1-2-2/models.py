...
def conv2d(x, W):
	return tf.nn.conv3d(
		x, W, strides=[1, 1, 1, 1 , 1], padding = 'SAME')
	# 1 [x_strides , y_strides , z_strides] 
	# padding:  output size + padding = input_size 

def max_pool_2x2(x):
	return tf.nn.max_pool3d(
		x, ksize=[1,2,2,1,1], strides=[1,2,2,1,1], padding = 'SAME')
	# 1 [x_strides , y_strides , z_strides] 1
...
def model1(x):
	...
	x_image = tf.reshape(x, [-1, 32, 32, 3 , 1])

	### conv layer 1 ####
	w_conv1 = init_weights([ 3 , 3 , 3 , 1 , 32])
	...
	### conv layer 2 ####
	w_conv2 = init_weights([ 3 , 3 , 3 , 32 , 64])
	...
	#### Flatten layer ####
	h_flat = tf.reshape(h_pool2, [ -1 , 8 * 8 * 64 * 3 ])

	#### dense1 ####
	w_fc1 = init_weights([ 8 * 8 * 64 *3, d1])
...
def model3(x):
	...
	### conv layer 2 ####
	w_conv2 = init_weights([3,3,3,32,32])
	b_conv2 = init_weights([32])
	h_conv2 = tf.nn.relu(conv2d(h_conv1,w_conv2) + b_conv2)
	
	### conv layer 3 ####
	w_conv3 = init_weights([3,3,3,32,32])
	b_conv3 = init_weights([32])
	h_conv3 = tf.nn.relu(conv2d(h_conv2,w_conv3) + b_conv3)

	#### Flatten layer ####
	h_flat = tf.reshape(h_conv3, [ -1 , 32*32*32*3 ])

	#### dense1 ####
	w_fc1 = init_weights([ 32*32 * 32*3 , d])
	b_fc1 = init_weights([d])
	h_fc1 = tf.nn.relu(tf.matmul(h_flat , w_fc1) + b_fc1)
