...
class im2txt:
	def __init__(self, parameters):
		...
		# weights = tf.Variable(tf.random_normal([2 * self.hidden_units, self.hidden_units]))
		# biases = tf.Variable(tf.random_normal([self.hidden_units]))
		# encoder_outputs = tf.reshape(encoder_outputs , [-1 , self.hidden_units*2])
		# encoder_outputs = tf.matmul(encoder_outputs , weights) + biases
		# encoder_outputs = tf.reshape(encoder_outputs , [-1 ,80 , self.hidden_units])
		...
		attention_states = tf.transpose(self.encoder_inputs, [0, 1, 2])
