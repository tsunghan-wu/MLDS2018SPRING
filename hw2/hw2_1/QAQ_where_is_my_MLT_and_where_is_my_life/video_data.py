...
##############################
def word_preprocessing(x , extraction = True):
	white_space = ['..', '...', '/']
	none = ['.', ',', '"', '\n', '?', '!', '(', ')']
...
class sentence_repair:
	...
	def can_be_verb(word):
		...
		for task in l:
			if task.pos() == 'v':
				...
				counting[1] +=1
		#print(counting)
		if counting[0]*1.5 > counting[1] or word in ['slice','attack' , 'dance' , 'stir']:
			#	print(word , 'is verb')
			return True
		return False			
	@staticmethod
		...
		if no_trivial:
			...
			for idx in range(len(sentence)):
				...
				if sentence_repair.can_be_verb(word)==False:
					if word in ['someone']:
						pass
					elif nltk.pos_tag([word])[0][1] == 'NN' and has_a == False:
			...
			if not has_the:
				sentence = sentence.split()
				for idx in range(len(sentence)):
					if sentence[idx] in ['in','of','at','with','on','to','by','into']:
						if idx+1 == len(sentence):
							# end of sentence
							sentence[idx] = sentence[idx] + ' the'
							break
						elif sentence[idx+1] not in ['a','an','some']:
							sentence[idx] = sentence[idx] + ' the'
							break
				sentence = ' '.join(sentence)

			sentence = sentence_repair.verb_repair(sentence)
			sentence = sentence_repair.specfic_repair(sentence)
			return sentence
	...
	@staticmethod
	def specfic_repair(sentence):
		out = []
		repair_list = {
			'tenni' : 'tennis',
			'cheesing' : 'cheese',
			'fingerring' : 'finger',
			'seasonning' : 'seasoning',
			'pawwing' : 'pawing',
			'usedding' : 'used',
			'ofs' : 'of',
			'ins' : 'in',
		...
		for word in sentence.split():
			if word in repair_list:
				word = repair_list[word]
			elif word[-1] == '\'':
				word = word+'s'
			out.append(word)
		return ' '.join(out)
...
class caption_data:
	...
	def __init__(self , word_min_frequency=3 , verbose = True , extraction=True):
		# read file
		...
		self.allData['caption'] = self.allData['caption'].apply(word_preprocessing,args=(extraction,))
